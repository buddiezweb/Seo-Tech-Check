...
function AdvancedResultsDisplay({ results, url, progress }) {
      ...
      <MetricGrid>
        ...
        <MetricCard background="#f0fff4">
          <MetricValue color="#48bb78">
            {Object.values(results).flat().filter(c => c?.status === 'pass').length}
          </MetricValue>
        ...
        <MetricCard background="#fef5e7">
          <MetricValue color="#f6ad55">
            {Object.values(results).flat().filter(c => c?.status === 'warn').length}
          </MetricValue>
        ...
        <MetricCard background="#fed7e2">
          <MetricValue color="#e53e3e">
            {Object.values(results).flat().filter(c => c?.status === 'fail').length}
          </MetricValue>
  ...
  const tabs = [
    ...
    { id: 'performance', label: 'Performance', content: () => renderChecks(results.performance) },
    { id: 'technical', label: 'Technical SEO', content: () => renderChecks(results.technical) },
    { id: 'content', label: 'Content', content: () => renderChecks(results.content) },
    { id: 'mobile', label: 'Mobile', content: () => renderChecks(results.mobile) },
    { id: 'security', label: 'Security', content: () => renderChecks(results.security) },
    { id: 'structured', label: 'Structured Data', content: () => renderChecks(results.structuredData) },
  ];
